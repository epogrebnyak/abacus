...
def test_income_statement(chart0, entries0):
    income_st = chart0.journal().post_many(entries0).close().income_statement()
...
def test_balance_sheet_with_close(chart0, entries0):
    balance_st = chart0.journal().post_many(entries0).close().balance_sheet()
...
def test_balances(chart0, entries0):
    b = chart0.journal().post_many(entries0).close().balances()
    assert b == {
        "cash": 1100,
        "receivables": 0,
        "goods_for_sale": 50,
