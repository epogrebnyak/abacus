...
here is a balance sheet `A = C + L`, it is a published financial statement; 
here is an income statement `P = I - E`, it is a published financial statement;
...
Why not look the accounting identity as `A = C + L + P` and start applying algebraic rules first before even knowing anything in accounting. Hope this approach can flatten the learning curve in accounting by making simple things simple again.
