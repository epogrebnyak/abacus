...
from engine.accounts import (Asset, Capital, ContraAsset, ContraIncome, RetainedEarnings, Liability, IncomeSummaryAccount, Expense, Income)
...
def test_closing_entries():
    ...
    assert list(
        make_closing_entries(chart, ledger).all()
    ) == [
