...
def cx_add(account_names):
    ...
    try:
        get_user_chart().use(*account_names).save(get_chart_path())
        print("Added to chart:", ", ".join(account_names))
...
def cx_post(debit, credit, amount):
    ...
    if not get_entries_path().exists():
        sys.exit(f"FileNotFoundError: {get_entries_path()}")
