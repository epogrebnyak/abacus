...
@click.group()
def cx():
    """Minimal tool for accounting cycle."""
...
def cx_command():
    ...
    exit_code=0
    cpath = get_chart_path()
    if cpath.exists():
        print(f"Chart file ({cpath}) already exists.")
        exit_code=1
    else:    
        user_chart().save(cpath)
    epath = get_entries_path()
    if epath.exists():
        print(f"Entries file ({epath}) already exists.")
        exit_code=1
    else:
        Path(epath).touch()     
    sys.exit(exit_code)
...
@cx.command(name="use")
def cx_use():
    """Import starting balances from JSON file."""
    pass


@cx.command(name="balances")
def cx_balances():
